...
export const authApi = {
  ...
  async resetPassword(token: string, password: string, confirmPassword: string): Promise<ApiResponse<null>> {
  ...
  async verifyEmail(token: string, email: string): Promise<ApiResponse<null>> {
    const res = await fetch(`${GATEWAY_URL}/api/v1/auth/verify-email?token=${encodeURIComponent(token)}&email=${encodeURIComponent(email)}`);
    return res.json();
  },

  async verifyStatus(email: string): Promise<ApiResponse<{ verified: boolean }>> {
    const res = await fetch(`${GATEWAY_URL}/api/v1/auth/verify-status?email=${encodeURIComponent(email)}`);
    return res.json();
  },
};
