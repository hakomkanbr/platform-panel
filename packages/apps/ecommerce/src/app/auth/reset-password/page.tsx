...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) { setSuccess(true); setTimeout(() => router.push("/auth/login"), 2000); }
      else { setError(result.error?.message || "Reset failed. The link may have expired."); }
  ...
  if (!token) {
    return (
      <div style={{ textAlign: "center", padding: "16px 0" }}>
        <div style={{ margin: "0 auto 14px", width: 48, height: 48, borderRadius: "50%", background: "#fef2f2", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#dc2626" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <circle cx="12" cy="12" r="10" /><line x1="15" y1="9" x2="9" y2="15" /><line x1="9" y1="9" x2="15" y2="15" />
          </svg>
        </div>
        <Text style={{ color: "#374151", fontSize: 15, fontWeight: 600, display: "block" }}>Invalid or expired link</Text>
        <div style={{ color: "#6b7280", fontSize: 13, marginTop: 6, marginBottom: 18, lineHeight: 1.5 }}>
          This password reset link is no longer valid.
        </div>
        <Link href="/auth/forgot-password">
          <Button type="primary" size="large"
            style={{ background: "linear-gradient(135deg, #6366f1, #8b5cf6)", border: "none", height: 42, borderRadius: 8 }}>
    ...
    <AnimatePresence mode="wait">
      {success ? (
        <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} style={{ textAlign: "center", padding: "8px 0" }}>
          ...
          <Text style={{ color: "#374151", fontSize: 16, fontWeight: 600, display: "block" }}>Password reset successful!</Text>
          <div style={{ color: "#6b7280", fontSize: 13, marginTop: 6 }}>Redirecting to login...</div>
        </motion.div>
      ) : (
        <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <div style={{ marginBottom: 6 }}>
            <Text style={{ color: "#374151", fontSize: 20, fontWeight: 700, display: "block", letterSpacing: "-0.03em" }}>Reset your password</Text>
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={{ color: "#374151", fontSize: 13, fontWeight: 500, display: "block", marginBottom: 6 }}>New password</label>
            <Input.Password size="large" placeholder="Enter new password" prefix={<LockOutlined style={{ color: "#9ca3af" }} />}
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <Input.Password size="large" placeholder="Repeat new password" prefix={<LockOutlined style={{ color: "#9ca3af" }} />}
          ...
          <div style={{ padding: 14, borderRadius: 10, background: "#fafafa", marginBottom: 20 }}>
            <Text style={{ color: "#6b7280", fontSize: 11, fontWeight: 500, marginBottom: 6, display: "block" }}>Password requirements</Text>
            {requirements.map((req) => (
              <div key={req.label} style={{ display: "flex", alignItems: "center", gap: 6, padding: "2px 0" }}>
                {req.test(password) ? (
                ...
                <Text style={{ color: req.test(password) ? "#22c55e" : "#6b7280", fontSize: 12 }}>{req.label}</Text>
              </div>
            ))}
          </div>
