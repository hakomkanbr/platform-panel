...
import { billingExtApi } from "@/lib/api/billing-ext";
import type {
  AppCatalogDto,
  ProjectDto,
  ProjectDetailDto,
...
export function useCurrentCapabilities(tenantId?: string) {
  return useQuery<Record<string, { value: number; capabilityType: string; unit: string | null; capabilityName: string }>, Error>({
